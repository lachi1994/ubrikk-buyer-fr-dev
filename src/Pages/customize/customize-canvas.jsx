...
class FabricCanvas extends Component {
	...
	avoidOverlap = (e) => {
		...
		this.canvas.forEachObject(function (obj) {
			...
			if (
				Math.abs(
					e.target.get('top') + e.target.get('height') - (obj.get('top') + obj.get('height'))
				) < snap
			) {
			...
			if (
				Math.abs(
					e.target.get('left') + e.target.get('width') - (obj.get('left') + obj.get('width'))
				) < snap
			) {
	...
	render() {
		return (
			<div className='w-100 flex-grow-1 d-flex justify-content-center align-items-center rounded'>
				<div
					className='canvas-area shadow-lg'
					style={{ maxWidth: '90%', height: this.props.state.canvasheight, overflow: 'hidden' }}>
